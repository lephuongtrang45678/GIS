<?php
    ...
    {
        // Kết nối CSDL
        $paPDO = new PDO('pgsql:host=localhost; dbname=testCSDL; port=5432','postgres','khanhoccho06');
        return $paPDO;
    }
    ...
    function getInfoCMRToAjax($paPDO,$paSRID,$paPoint)
    {
        ...
        //$mySQLStr = "SELECT ST_AsGeoJson(geom) as geo from \"CMR_adm1\" where ST_Within('SRID=4326;POINT(12 5)'::geometry,geom)";
        //$mySQLStr = "SELECT ST_AsGeoJson(geom) as geo from \"CMR_adm1\" where ST_Within('SRID=".$paSRID.";".$paPoint."'::geometry,geom)";
        $mySQLStr = "SELECT id_1,name_1, shape_leng, shape_area from \"cmr_adm1\" where ST_Within('SRID=".$paSRID.";".$paPoint."'::geometry,geom)";
        //echo $mySQLStr;
        //echo "<br><br>";
        $result = query($paPDO, $mySQLStr);
        ...
        if ($result != null)
        {
            ...
            foreach ($result as $item){
                ...
                $resFin = $resFin.'<tr><td>Tên lãnh thổ: '.$item['name_1'].'</td></tr>';
